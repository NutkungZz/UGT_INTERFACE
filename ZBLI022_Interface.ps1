# ZBLI022_Interface.ps1
# Script for importing SAP ZBLI022 data from FTP server to database
# -----------------------------------------------------------------------------
...
$ftpPath = "/interface/3rdparty/UGT1/Outbound/ZBLI022/"
...
$logFileName = "ZBLI022_Import_Log_$timestamp.txt"
...
function Test-FileProcessed {
    ...
    try {
        $query = "SELECT COUNT(*) FROM [dbo].[ZBLI022_OUTBOUND] WHERE [FILE_NAME] = @fileName"
...
# Function to process the ZBLI022 text file
function Process-Zbli022File {
    param (
        [string]$FilePath,
        [string]$FileName,
        [System.Data.SqlClient.SqlConnection]$Connection
    ...
    try {
        ...
        foreach ($line in $fileContent) {
            if (-not [string]::IsNullOrWhiteSpace($line)) {
                ...
                $ba = $fields[1].Trim()
                $ca = $fields[2].Trim()
                $installation = $fields[3].Trim()
                $unitUsed = [double]::Parse($fields[4].Trim())
                $readingDate = Convert-SapDateToSql -SapDate $fields[5].Trim()
                $billPeriod = $fields[6].Trim()
                
                # Insert into database
                $query = @"
INSERT INTO [dbo].[ZBLI022_OUTBOUND]
    ([TRSG], [BA], [CA], [INSTALLATION], [UNIT_USED], [READING_DATE], [BILL_PERIOD], [FILE_NAME])
VALUES
    (@trsg, @ba, @ca, @installation, @unitUsed, @readingDate, @billPeriod, @fileName)
"@
...
try {
    Write-Log "Starting ZBLI022 import process..."
    ...
    $dataFiles = $ftpFiles | Where-Object { $_ -like "ZBLI022_*.txt" -and $_ -notlike "*.OK" }
    
    Write-Log "Found $($dataFiles.Count) ZBLI022 data files on FTP server"
    ...
    foreach ($dataFile in $dataFiles) {
        ...
        $recordCount = Process-Zbli022File -FilePath $localFilePath -FileName $dataFile -Connection $connection
    ...
    Write-Log "ZBLI022 import process completed successfully"
}
catch {
